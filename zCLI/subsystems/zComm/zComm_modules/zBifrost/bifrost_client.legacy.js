...
(function(root, factory) {
  ...
  class BifrostClient {
    ...
    constructor(url = 'ws://localhost:8765', options = {}) {
      ...
      // Client-side cache orchestrator (v1.5.4+) - Mirrors zLoader's 4-tier cache
      this.cache = null;
      this._initCache(options);
      
      // Legacy schema cache for backward compatibility (until cache is ready)
    ...
    /**
     * Initialize cache orchestrator (async, happens in background)
     * @private
     */
    async _initCache(options) {
      try {
        // Try to load cache modules (browser script tag already loaded them)
        if (typeof window !== 'undefined' && window.CacheOrchestrator) {
          const CacheOrchestrator = window.CacheOrchestrator;
          
          this.cache = new CacheOrchestrator({
            debug: options.debug,
            persistCache: options.persistCache !== false,
            maxSize: options.maxCacheSize || 100,
            defaultTTL: options.defaultCacheTTL || 3600000 // 1 hour
          });
          
          // Migrate legacy schema cache if any
          if (this.schemaCache.size > 0) {
            for (const [key, value] of this.schemaCache.entries()) {
              await this.cache.set(key, value, 'system', {
                metadata: { type: 'schema', migratedFromLegacy: true }
              });
            }
            this._log('✅ Cache system initialized, migrated', this.schemaCache.size, 'schemas');
          } else {
            this._log('✅ Cache system initialized (4-tier: system, pinned, plugin, session)');
          }
        } else {
          // Try dynamic import for Node.js or module-based environments
          const module = await import('./bifrost_client_modules/cache_orchestrator.js');
          const CacheOrchestrator = module.default || module.CacheOrchestrator;
        ...
        // Fallback to legacy in-memory cache if module loading fails
        console.warn('[BifrostClient] Cache system unavailable, using legacy in-memory cache:', e);
      }
    }
    ...
    async getSchema(model) {
      // Use new cache system if available, otherwise legacy
      if (this.cache) {
        // Check system cache first (fast path)
        const cached = await this.cache.get(model, 'system');
        if (cached) {
          this._log('📦 Schema cache hit:', model);
          return cached;
        }
      } else if (this.schemaCache.has(model)) {
        // Legacy cache fallback
        this._log('📦 Schema cache hit (legacy):', model);
        return this.schemaCache.get(model);
      }

      // Request from server (zLoader will also cache on backend)
      ...
      if (this.cache) {
        await this.cache.set(model, schema, 'system', {
          ttl: 3600000, // 1 hour
    ...
    clearClientCache() {
      ...
      // Clear new cache system if available
      if (this.cache) {
        this.cache.clearAll();
      }
      
      this._log('🗑️ All client caches cleared');
    }
     ...
     */
    async clearCache(cacheType = 'system') {
      if (this.cache) {
        await this.cache.clear(cacheType);
        this._log(`🗑️ ${cacheType} cache cleared`);
      } else {
        console.warn('[BifrostClient] Cache system not initialized');
      }
    }

    /**
     * Get all cache statistics (client-side only)
     * @returns {Object} Cache statistics for all 4 cache types
     */
    getAllCacheStats() {
      if (this.cache) {
    ...
    /**
     * Pin/bookmark a resource for persistent access
     * @param {string} alias - User-defined alias
     * @param {string} resourcePath - Path to resource
     * @returns {Promise<any>} The pinned data
     */
    async pinResource(alias, resourcePath) {
      if (!this.cache) {
        throw new Error('[BifrostClient] Cache system not initialized');
      }
      ...
      // Pin it to pinned cache
      await this.cache.set(alias, data, 'pinned', {
        metadata: { path: resourcePath, pinnedAt: Date.now() }
      });
      
      this._log(`📌 Pinned: ${alias} => ${resourcePath}`);
      return data;
    }
     ...
     */
    getPinnedResource(alias) {
      if (this.cache) {
        return this.cache.get(alias, 'pinned');
      }
      return null;
    }
    ...
    async loadPlugin(pluginName, url) {
      ...
      return await this.cache.pluginCache.loadAndCache(pluginName, url);
    }
