...
class BasicData:
    """Basic data display events: list, json.
    ...
    def list(self, items, style="bullet", indent=0):
        ...
        Terminal: Formatted list with bullets/numbers
        GUI: Clean event with items array for custom rendering
    ...
    def json_data(self, data, indent_size=2, indent=0, color=False):
        ...
        Terminal: Pretty-printed JSON with optional syntax coloring
        GUI: Clean event with raw data object for frontend rendering
        
        Args:
            data: Data to serialize as JSON
            indent_size: JSON indentation size (spaces)
            indent: Base indentation level
            color: Apply syntax coloring (terminal only)
        """
    ...
    def _colorize_json(self, json_str):
        """Apply basic syntax coloring to JSON string (terminal only).
