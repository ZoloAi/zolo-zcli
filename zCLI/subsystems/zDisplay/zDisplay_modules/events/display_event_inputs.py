...
class BasicInputs:
    ...
    def selection(self, prompt, options, multi=False, default=None, style="numbered"):
        """Display selection prompt and return user's choice(s) - single option or list for multi-select."""
    ...
    def _process_selection_number(self, num_str, options, selected):
        """Process a single selection number input."""
        try:
            idx = int(num_str) - 1
