...
**1. Factory Pattern (Recommended):**
    >>> from zCLI.subsystems.zData.zData_modules.shared.backends import AdapterFactory
    >>> config = {"path": "/data/myapp", "label": "sqlite"}
    >>> adapter = AdapterFactory.create_adapter("sqlite", config)
    >>> adapter.connect()

**2. Direct Adapter Import:**
    >>> from zCLI.subsystems.zData.zData_modules.shared.backends import SQLiteAdapter
    >>> config = {"path": "/data/myapp", "label": "sqlite"}
    >>> adapter = SQLiteAdapter(config, logger=logger)
    >>> adapter.connect()

**3. Plugin Registration (Custom Adapters):**
    >>> from zCLI.subsystems.zData.zData_modules.shared.backends import (
