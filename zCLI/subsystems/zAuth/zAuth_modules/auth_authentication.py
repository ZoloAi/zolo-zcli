    ...
    **Usage:**
    ...
    def login(
        ...
        if not username or not password:
            ...
            if creds:
                username = creds.get(KEY_USERNAME)
                password = creds.get(KEY_PASSWORD)
            else:
            ...
            if result.get(KEY_STATUS) == STATUS_SUCCESS:
                ...
                if credentials and self.session:
                    ...
                    self.zcli.display.zEvents.zAuth.login_success({
                        KEY_USERNAME: credentials.get(KEY_USERNAME),
                        FIELD_ROLE: credentials.get(FIELD_ROLE),
                        "user_id": credentials.get("user_id"),
                        FIELD_API_KEY: credentials.get(FIELD_API_KEY)
                    })
        ...
        self.zcli.display.zEvents.zAuth.login_failure(ERR_INVALID_CREDS)
